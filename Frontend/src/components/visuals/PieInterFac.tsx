...
export function PieInteractive({ institute_id }: PieInteractiveProps) {
  ...
  React.useEffect(() => {
    const fetchDepartmentWiseStudentCount = async () => {
      try {
        console.log(
          "Fetching department-wise faculty count for institute_id:",
          institute_id
        );
  ...
  return (
    <Card className="flex flex-col">
      <CardHeader className="items-center pb-0">
        <CardTitle>Department-wise Faculty Count</CardTitle>
        <CardDescription>Total Faculties: {totalStudents}</CardDescription>
      </CardHeader>
      <CardContent>
        <ChartContainer
