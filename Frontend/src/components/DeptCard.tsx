...
export function DeptCard({
  ...
  const [updatedCoordinatorEmail, setUpdatedCoordinatorEmail] = useState(coordinator_email);
  const [emails, setEmails] = useState<string[]>([]);

  useEffect(() => {
    const fetchEmails = async () => {
      try {
        const response = await fetch(`http://localhost:3000/api/facultyEmails/${institute_id}`);
        if (!response.ok) {
    ...
    <>
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        ...
        <DialogContent className="p-6 max-w-lg mx-auto">
          <ScrollArea>
          <DialogHeader>
            <DialogTitle className="text-2xl font-semibold mb-4">Edit Department</DialogTitle>
          </DialogHeader>
          <div className="space-y-4 mb-4">
            ...
            <div>
                    <label htmlFor="offer" className="block mb-1">Coordinator Email</label>
                    <Select name="host" value={updatedCoordinatorEmail} onValueChange={(value) => setUpdatedCoordinatorEmail((value))}>
          ...
          <div className="flex flex-row justify-between w-full space-x-4">
            <Button className="flex-1 mt-4 py-2 px-4 rounded" onClick={handleUpdate}>
              Update
            </Button>
            <Button variant="destructive" className="flex-1 mt-4 py-2 px-4 rounded" onClick={handleDelete}>
              <Trash2 /> Delete
            </Button>
          </div>
          </DialogClose>
